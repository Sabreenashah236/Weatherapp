...
function App() {
  ...
  const fetchWeatherInfo = (e) => {
    ...
    const options = {
      ...
      params: {
        q: inputRef.current.value || "cardiff, uk", // "cardiff, uk"
        units: "metric",
      },
      headers: {
  ...
  const determineBackgroundImage = () => {
    if (weatherInfo?.main.temp < 10) {
      setImage(
        "https://images.unsplash.com/photo-1555588746-13edde80e4a1?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=2734&q=80"
      );
    }
